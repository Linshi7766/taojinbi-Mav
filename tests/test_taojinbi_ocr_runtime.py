...
class StartupPopupRecoveryTests(unittest.TestCase):
    ...
    def test_entry_failure_emits_recovery_fingerprint(self):
        ...
        with patch.object(runtime, "on_task_list", return_value=False),              patch.object(runtime, "_reopen_task_popup", return_value=False),              patch.object(runtime, "_safe_back_to_coin_page", return_value=False),              patch.object(runtime, "ocr_screen", return_value=stuck),              patch.object(runtime, "run_safe_browse_tasks") as run:
            outcome = runtime._execute_scan(
                device, None, 1, logger, RunMode.EXECUTE,
            )
        self.assertEqual(
