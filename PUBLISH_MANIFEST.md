...
验证：2026-08-31 复验（代码树 HEAD `e9e9265`；命令 `PYTHONPATH=src python -m
unittest discover -s tests -t .`；Python 3.11 / Windows），导出目录离线测试
**448/448** 通过；两个入口 `--help` 正常；敏感扫描零命中；私有/公开 CI
（Windows + Ubuntu）全绿。此前"309/309"记录（2026-08-28）已过时，以本次为准。
