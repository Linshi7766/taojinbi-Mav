...
验证：2026-09-02 复验（代码树 HEAD `9bbf5d2`；命令 `PYTHONPATH=src python -m
unittest discover -s tests -t .`；Python 3.13 / Windows），导出目录离线测试
**527/527** 通过；私有/公开 CI（Windows + Ubuntu）全绿。此前"448/448"记录
（2026-08-31）已过时，以本次为准。
